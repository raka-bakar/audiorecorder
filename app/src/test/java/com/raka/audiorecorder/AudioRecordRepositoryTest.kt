...
class AudioRecordRepositoryTest {
    ...
    private val expected = AudioRecord(filename = "filename", filepath = "file://filepath",
        duration = "00:00:01")
    ...
    @Test
    fun `verify the correct data is sent to database when adding a new audio record`()= runBlocking {
        sut.addAudioRecord(expected)
        verify(dataSource).addAudioRecord(expected)
    }

    @Test
    fun `verify the correct data is sent when deleting an audio record`()= runBlocking {
